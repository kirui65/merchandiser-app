...
export default function ProductsPage() {
  ...
  return <><div className="page-heading"><span className="eyebrow">CATALOG</span><h1>Products</h1><p className="muted">Maintain the field-sales catalog without removing historical sales records.</p></div>{error && <div className="error-banner">{error}</div>}<div className="grid-2"><Card title="Create product"><form className="form-grid" onSubmit={submit}>{productForm(form, setForm)}<Button type="submit">Create product</Button></form></Card><Card title={`${products.length} products`}>{products.length ? <div className="table-wrap"><table><thead><tr><th>Name</th><th>SKU</th><th>Category</th><th>Price</th><th>Status</th><th>Actions</th></tr></thead><tbody>{products.map((product) => <tr key={product.id}><td>{product.name}</td><td>{product.sku}</td><td>{product.category}</td><td>{formatKes(product.defaultPrice)}</td><td>{product.active === false ? 'Inactive' : 'Active'}</td><td><Button variant="secondary" onClick={() => beginEdit(product)}>Edit</Button>{' '}<Button variant="secondary" onClick={() => toggle(product)}>{product.active === false ? 'Reactivate' : 'Deactivate'}</Button></td></tr>)}</tbody></table></div> : <EmptyState title="No products yet" message="Create the first product for the sales catalog." />}</Card></div>{editing && <div className="ui-card" style={{ marginTop: 18 }}><h2>Edit {editing.name}</h2><form className="form-grid" onSubmit={saveEdit}>{productForm(edit, setEdit)}<div><Button type="submit">Save changes</Button>{' '}<Button type="button" variant="secondary" onClick={() => setEditing(null)}>Cancel</Button></div></form></div>}</>;
}
