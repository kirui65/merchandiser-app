...
export default function ShiftHistoryScreen() {
  ...
  useEffect(() => { getRouteHistory().then(setRoutes).catch(() => setError('Unable to load shift history.')); }, []);
  ...
  return <SafeAreaView style={styles.container}><View style={styles.header}><Text style={styles.eyebrow}>PAST SHIFTS</Text><Text style={styles.title}>Shift history</Text><Text style={styles.subtitle}>Distance and outlet coverage from your recorded routes.</Text></View><FlatList data={routes} keyExtractor={(route) => route.id} contentContainerStyle={styles.list} renderItem={({ item }) => { const summary = shiftSummary(item); return <Card style={styles.card}><Text style={styles.date}>{item.date}</Text>{summary.start ? <Text style={styles.time}>{new Date(summary.start).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} – {new Date(summary.end).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</Text> : <Text style={styles.time}>No GPS pings recorded</Text>}<View style={styles.stats}><View><Text style={styles.value}>{formatDuration(summary.durationMinutes)}</Text><Text style={styles.label}>Duration</Text></View><View><Text style={styles.value}>{summary.distance.toFixed(1)} km</Text><Text style={styles.label}>Distance</Text></View><View><Text style={styles.value}>{item.visitedOutletIds?.length || 0}</Text><Text style={styles.label}>Visited</Text></View></View></Card>; }} ListEmptyComponent={<EmptyState icon="◷" title="No past shifts" message="Completed shifts with GPS activity will appear here." />} /></SafeAreaView>;
}

const styles = StyleSheet.create({ container: { flex: 1, backgroundColor: colors.background }, loading: { flex: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.background, gap: spacing.sm }, loadingText: { color: colors.muted, fontSize: typography.small }, header: { padding: spacing.lg, paddingBottom: spacing.sm }, eyebrow: { color: colors.primary, fontSize: 11, fontWeight: '800', letterSpacing: 1.2 }, title: { color: colors.ink, fontSize: typography.title, fontWeight: '800', marginTop: spacing.xs }, subtitle: { color: colors.muted, fontSize: typography.small, lineHeight: 19, marginTop: spacing.xs }, list: { flexGrow: 1, padding: spacing.lg, paddingTop: spacing.sm }, card: { marginBottom: spacing.md }, date: { color: colors.ink, fontSize: typography.h3, fontWeight: '800' }, time: { color: colors.muted, fontSize: typography.small, marginTop: 4 }, stats: { flexDirection: 'row', justifyContent: 'space-between', marginTop: spacing.md }, value: { color: colors.primary, fontSize: typography.body, fontWeight: '800' }, label: { color: colors.muted, fontSize: 11, marginTop: 3 } });
